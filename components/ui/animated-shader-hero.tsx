...
const badgeIconClasses = ['text-yellow-300', 'text-orange-300', 'text-amber-300'] as const
...
void main(){gl_Position=position;}`
  ...
  class PointerHandler {
    ...
    constructor(element: HTMLCanvasElement, scale: number) {
      ...
      const map = (target: HTMLCanvasElement, nextScale: number, x: number, y: number) =>
        [x * nextScale, target.height - y * nextScale]
...
export default function Hero({
  ...
  return (
    <div className={`relative w-full min-h-screen overflow-hidden bg-black ${className}`}>
      ...
      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center px-4 py-10 text-white">
        {hideDefaultContent ? (
          children
        ) : (
          <>
                  ...
                  {trustBadge.icons ? (
                    <div className="flex">
                      {trustBadge.icons.map((icon, index) => (
                        <span
                          key={`${icon}-${index}`}
                          className={badgeIconClasses[index] || badgeIconClasses[badgeIconClasses.length - 1]}
                        >
                          {icon}
                        </span>
                      ))}
            ...
            <div className="mx-auto max-w-5xl space-y-6 text-center">
              ...
              {children ? (
                <div className="mx-auto mt-10 w-full max-w-md animate-fade-in-up animation-delay-800">
                  {children}
                </div>
              ) : null}
            </div>
